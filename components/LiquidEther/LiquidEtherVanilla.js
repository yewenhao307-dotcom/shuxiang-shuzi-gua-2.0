...
export function mountLiquidEther(hostElement, options = {}) {
  const {
    mouseForce = 20,
    cursorSize = 100,
    isViscous = false,
  ...
  const mountRef = { current: hostElement };
  ...
  Object.assign(hostElement.style, style);
  hostElement.classList.add('liquid-ether-container');
  if (className) hostElement.classList.add(...className.split(/\s+/).filter(Boolean));
...
export default mountLiquidEther;
